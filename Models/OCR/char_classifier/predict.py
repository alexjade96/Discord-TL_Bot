# Single-image character inference.
# Run from OCR/:
#   python -m char_classifier.predict --image path/to/char.png
#   python -m char_classifier.predict --image path/to/char.png --model ../checkpoints/best.pt --top-k 5
import argparse
import json
...
def predict(
    ...
    """Return top-k [(character_label, confidence), ...] for an image."""
...
def main():
    p = argparse.ArgumentParser(description='Character OCR Inference')
    ...
    print(f'Character predictions for: {args.image}')
    print('-' * 50)
    for rank, (label, conf) in enumerate(predictions, 1):
        bar = chr(9608) * int(conf * 30)
        print(f'  {rank:2d}. {label:40s}  {conf:.2%}  {bar}')
