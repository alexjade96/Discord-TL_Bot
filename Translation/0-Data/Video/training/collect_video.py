"""Collect Discord video submissions and transcription results for training.

Each successfully transcribed submission is appended to data/video_submissions.jsonl.
...
Schema:
    ...
    filename            -- original Discord video filename (e.g. clip.mp4)
    transcript          -- Whisper output text (from extracted audio)
...
SUBMISSIONS_FILE = DATA_DIR / "video_submissions.jsonl"
...
def save_submission(
    ...
    """Save a video transcription + translation submission to the dataset.
...
def load_submissions() -> list[dict]:
    """Return all collected video submissions as a list of dicts."""
    if not SUBMISSIONS_FILE.exists():
        return []
    with SUBMISSIONS_FILE.open(encoding="utf-8") as f:
...
def dataset_stats() -> dict:
    """Return a summary of the current video dataset."""
